...
function UnitFrames:Target()
	...
	if IsPat(UnitName"Player") then
		...
		if C["UnitFrames"]["Portrait"] == true then
			...
			Portrait:Point("BOTTOMLEFT", ufbg, "BOTTOMRIGHT", 6, 2)
		...
		if (C["UnitFrames"]["TargetAuras"]) then
			...
			Buffs:SetPoint("BOTTOMLEFT", self, "TOPLEFT", -1, 6)
			...
			Debuffs:SetPoint("BOTTOMRIGHT", Buffs, "TOPRIGHT", 6, 3)
		...
		if C["UnitFrames"]["Portrait"] == true then
			Portrait:Size(40, 23)
			Portrait:ClearAllPoints()
			Portrait:Point("LEFT", Health, "RIGHT", 8, 0)
